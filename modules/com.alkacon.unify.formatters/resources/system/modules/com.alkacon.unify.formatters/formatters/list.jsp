	...
	<c:if test="${'' ne con.value.SortOrder && '' ne con.value.RowCount && '' ne con.value.CreatePath}">
		...
		<c:choose>
			<c:when test="${cms.element.inMemoryOnly}">
				<div class="alert"><fmt:message key="unify.list.message.new" /></div>
			</c:when>
			<c:when test="${empty collectorParam}">
				<div class="alert"><fmt:message key="unify.list.message.edit" /></div>
			</c:when>
			<c:otherwise>
            	...
            	<c:when test="${info.resultSize > 0}">
              	...
              	<div id="list_large_pages">
        					<div id="list_large_page_1">
        						<cms:include file="%(link.weak:/system/modules/com.alkacon.unify.formatters/elements/list-singlepage.jsp:7db0254c-5872-11e5-b803-0242ac11002b)">
