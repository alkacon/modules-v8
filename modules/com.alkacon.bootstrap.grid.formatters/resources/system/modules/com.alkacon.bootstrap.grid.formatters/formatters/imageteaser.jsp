	...
	<div ${content.rdfa["Link|Image"]}>				
		<div class="thumbnails thumbnail-style thumbnail-kenburn">
			<c:if test="${value.Image.isSet}"><div class="thumbnail-img">
				...
				<c:if test="${value.Link.isSet}"><a class="btn-more hover-effect" href="<cms:link>${value.Link}</cms:link>"><fmt:message key="bootstrap.imageteaser.frontend.readmore" /></a></c:if>
				</div></c:if>
			<div class="caption">
				<c:choose>
				...
				<c:if test="${not value.Image.isSet and value.Link.isSet}"><div style="text-align: right; margin-top: 20px;"><a class="btn-more hover-effect" style="position: relative;" href="<cms:link>${value.Link}</cms:link>"><fmt:message key="bootstrap.imageteaser.frontend.readmore" /></a></div></c:if>
			</div>
		</div>
	</div>
